...
def update_map(current_step, path_length, total_moves, dir, location, map):
    ...
    if total_moves != 0:
        ...
        if current_step % move_step == 0:  # 整数倍关系时需要更新地图
            tmp_map = np.roll(
                map, move, axis=(0, 1)
            )  # 按照目标运动方向移动地图（目标转移概率地图）
            map = tmp_map / np.sum(tmp_map)  # Scale it to 1

    p_sensor_no_detection = np.ones(
        (map_size_x, map_size_y)
    )  # Initialize the probability of no detection with ones
    p_sensor_no_detection[int(location["y"]), int(location["x"])] = (
        0  # For binary sensor model, the probability of no detection at UAV location is zero
    )
...
if __name__ == "__main__":
    ...
    # 更新地图
    scale_factor, new_map = update_map(
        current_step, path_length, total_moves, dir, location, map
    )
